...
@Component({
  ...
  styleUrls: ['./crear-instalacion.component.scss']
})
export class CrearInstalacionComponent implements OnInit {
  ...
  createInstalacionModel = this.formBuilder.group({
  ...
  ngOnInit(): void {
    this.createInstalacionModel.reset();
  }

  onSubmit() {
    this.service.createInstalacion(this.createInstalacionModel).subscribe(
      res => {
        this.createInstalacionModel.reset();
        this.router.navigate(['/activos/instalaciones']);

        this.toastr.success('La instalación se ha creada exitosamente','Instalación creada');
      },
      err => {
